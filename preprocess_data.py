...
copy_file = os.path.join('data', 'raw', 
...
# Process RNAseq data by z-score and zeroone norm
...
# ### Mutation
#
# Mutation data are stored in a long format. 
# First, subset the data to only deleterious mutations listed below. 
# Next, pivot the dataframe to have 
# samples as the index, genes as the columns, and 
# either a 1 or 0 to indicate a deleterious mutation or wild-type sample by gene.
...
# ### Copy Number
# 
# Copy number data contains thresholded GISTIC2.0 calls 
# where 0 equals wild-type copy number, 
#       1 and -1 mean slight gain and slight loss, respectively, and 
#       2 and -2 mean high gain and high loss, respectively.
...
# For our purposes, a copy loss status event (1 vs. 0) 
# is conservatively defined only as a deep loss.
...
clinical_sub_df = clinical_df.rename(columns=clinical_columns_dict)
...
# ### OncoKB Gene-Types
# 
# Here, we use the [OncoKB API](http://oncokb.org/#/dataAccess) 
# of [Chakravarty et al. 2017]
# (http://ascopubs.org/doi/abs/10.1200/JCO.2016.34.15_suppl.11583) 
# to download all oncogenes and tumor suppressor genes. 
# These help to subset the copy number gain and copy loss data frames 
# to identify a full status matrix.

# Output File
oncokb_out_file = os.path.join('data', 'oncokb_genetypes.tsv')

print("Processing OncoKB data ...")
...
print("Producing Full Status Matrix ...")
...
status_gain = copy_gain_df.reindex(columns = oncogenes_df['hugoSymbol'])
status_loss = copy_loss_df.reindex(columns = tsg_df['hugoSymbol'])
...
mutation_status = mut_pivot.reindex(index = status_samples)
copy_status = copy_status.reindex(index = status_samples)
